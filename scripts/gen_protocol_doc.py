...
def generate() -> str:
    ...
    sections = [
        ...
        "### session.status\n\n",
        _model_section("SessionStatusCommand", SessionStatusCommand),
        _model_section("SessionStatusResult", SessionStatusResult),
        "### session.resume\n\n",
        _model_section("SessionResumeCommand", SessionResumeCommand),
        _model_section("SessionResumeResult", SessionResumeResult),
        "### session.review\n\n",
        _model_section("SessionReviewCommand", SessionReviewCommand),
        _model_section("SessionResumeResult", SessionResumeResult),
        "### permission.respond\n\n",
        ...
        "A user submission should keep the same `request_id` when retried. "
        "The key is scoped to its session; changing the content conflicts. "
        "The JSON-RPC `id` is only a transport correlation ID. "
        "`session.send_message` returns after durable acceptance; completion arrives as events.\n\n",
        "Recovery never replays calls with unknown effects. "
        "`session.review` records `pause` or `abandon` with a required note. "
        "Abandon closes the session and retains unresolved facts; it does not undo effects. "
        "Approval replies must carry the fresh `approval_id` from the current daemon event.\n\n",
        "\n## Server Push\n\n",
